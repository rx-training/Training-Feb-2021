// There is a retail shop which need to manage 
//the inventory, whenever some purchase is being 
//made product quantity should be reduced, if 
//quanity is less than 5 reorder request should 
//be raised.
...
let purchaseData : Array<PurchaseDetail> = [];
...
function PurchageProduct(CID:number,PID:number,Qnt:number){
    ...
    if(p != null){
        if(p.quntityAvailable > Qnt){
            let pd:PurchaseDetail = {CustomerId:c.customerId,ProductId:p.productId,ProductName:p.productName,Date: new Date().toDateString(),Qntity: Qnt,Price: (Qnt * p.pricePerUnit)};
...
function ReOrder(PID:number , Qnt:number){
    let p = products.filter(i=>i.productId == PID)[0];
    let index = products.indexOf(p);
