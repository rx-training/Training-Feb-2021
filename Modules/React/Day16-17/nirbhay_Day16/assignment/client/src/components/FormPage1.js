...
export default function formPage1(props) {
  const { fname, mname, lname, pob, flang, dob, country, state, city } =
    props.student;
  const handleChange = props.handleChange;
  const validate = props.validate;
  const chooseCountry = props.chooseCountry;
  const chooseState = props.chooseState;
  const chooseCity = props.chooseCity;
