...
const OutstationDriverCard = (props) => {
  const [Driver, setDriver] = useState([]);

  useEffect(() => {
    DriverService.getOutstationDriver().then((res) => {
      setDriver(res.data);
    });
  }, []);

  const handleEdit = (id) => {
    props.history.push(`/EditOutstationDriver/${id}`);
  };

  const handleDelete = (id) => {
    DriverService.deleteOutstationDriver(id).then((res) => {
...
export default OutstationDriverCard;
