...
const CityDriverCards = (props) => {
  const [Driver, setDriver] = useState([]);
  console.log(Driver);

  useEffect(() => {
    DriverService.getCityDriver().then((res) => {
      setDriver(res.data);
  ...
  const handleEdit = (id) => {
    props.history.push(`/EditCityForm/${id}`);
  };

  const handleDelete = (id) => {
    DriverService.deleteCityDriver(id).then((res) => {
...
export default CityDriverCards;
