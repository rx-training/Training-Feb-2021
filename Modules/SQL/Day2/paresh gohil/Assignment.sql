...
/*UPDATE*/
	...
	/*Write a SQL statement to increase the minimum and maximum salary of PU_CLERK by 2000 as well as the salary for those employees by 20% and commission by 10% .*/
		...
		SELECT * FROM Employees
		UPDATE Employees SET Salary = Salary + (Salary*.20) + 2000 , CommissionPct = (CommissionPct + .10) WHERE JobId = 'PU_CLERK' AND Salary = (SELECT MAX(Salary) FROM Employees WHERE JobId = 'PU_CLERK')

		UPDATE Employees SET Salary = Salary +2000 + (Salary*.20) , CommissionPct = (CommissionPct + .10) WHERE JobId = 'PU_CLERK' AND Salary = (SELECT MIN(Salary) FROM Employees WHERE JobId = 'PU_CLERK')
