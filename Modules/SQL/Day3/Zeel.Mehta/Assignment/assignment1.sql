...
SELECT FirstName,LEN(FirstName) Length_FName FROM Employees 
...
SELECT LastName,LEN(FirstName)LENGTH_FNAME FROM Employees WHERE LastName LIKE '_C%'
GO 
...
SELECT DISTINCT(
     (CONCAT(DATEPART(WEEKDAY,HireDate),'1'))
	 ) 
          FROM Employees;
GO
