...
/*   5) Find the names of all salespeople who are managed by a salesperson named "Adam Smith".*/
...
/*   7) Find the total number of dealerships.*/

		WITH NumberOfDealership(NumberOfDealership) AS
		(SELECT COUNT(DealersipID) AS NumberOfDealerShip FROM Dealership)

/*  8) List the VIN, year, and mileage of all "Toyota Camrys" in the inventory of the dealership named "Toyota Performance".
...
/*   9)  Find the name of all customers who bought a car at a dealership located in a state other than the state in which they live.*/
		WITH Demo (Name) AS
...
/*   10)  Find the name of the salesperson that made the largest base salary working at the dealership named "Ferrari Sales" during January 2010.*/
...
/*   11)  List the name, street address, city, and state of any customer who has bought more than two cars from all dealerships combined since January 1, 2010.*/
...
/*  12)  List the name, salesperson ID, and total sales amount for each salesperson who has ever sold at least one car. The total sales amount for a salesperson is the sum of the sale prices of all cars ever sold by that salesperson.*/
...
/*   13) Find the names of all customers who bought cars during 2010 who were also salespeople during 2010. For the purpose of this query, assume that no two people have the same name.*/
...
/*   14)   Find the name and salesperson ID of the salesperson 
  ...
  /* 15) Calculate the payroll for the month of March 2010.
