...
SELECT FirstName,LastName FROM Employees WHERE ManagerID IN (SELECT DepartmentID FROM Employees WHERE DepartmentID IN (SELECT DepartmentID FROM Departments WHERE LocationID IN (SELECT LocationID FROM Locations WHERE CountryID = 'US')))

---- 4. Find the names (first_name, last_name) of the employees who are managers. */

SELECT FirstName,LastName FROM Employees WHERE ManagerID IN (SELECT DepartmentID FROM Employees)
...
SELECT FirstName,LastName,Salary FROM Employees WHERE Salary > (SELECT AVG(Salary) FROM Employees) AND DepartmentID = (SELECT DepartmentID FROM Departments WHERE DepartmentName = 'IT')
...
SELECT e.FirstName,e.LastName FROM Employees e WHERE NOT EXISTS (SELECT 'X' FROM Employees m WHERE m.ManagerID = e.EmployeeID)

---- 13. Write a query to display the employee ID, first name, last names, and department names of all employees. ------

SELECT  e.EmployeeID,e.FirstName,e.LastName,(SELECT d.DepartmentName FROM Departments d WHERE d.DepartmentID = e.DepartmentID)DepartName FROM Employees e 
...
SELECT EmployeeID,FirstName,LastName,Salary FROM Employees WHERE Salary > ALL(SELECT AVG(Salary) FROM Employees GROUP BY DepartmentID)

---- 15. Write a query to fetch even numbered records from employees table. ------

SELECT * FROM (SELECT * FROM Employees)EmployeeID WHERE EmployeeID % 2 = 0 ORDER BY EmployeeID

---- 16. Write a query to find the 5th maximum salary in the employees table. ------

SELECT * FROM (SELECT Salary,DENSE_RANK() OVER(ORDER BY Salary DESC) AS 'Salarys' FROM Employees)tbl WHERE Salarys = 5 

---- 17. Write a query to find the 4th minimum salary in the employees table. ------

SELECT * FROM (SELECT Salary,DENSE_RANK() OVER(ORDER BY Salary ASC) AS 'Salarys' FROM Employees)tbl WHERE Salarys = 4 

---- 18. Write a query to select last 10 records from a table. ------

SELECT * FROM (SELECT *,ROW_NUMBER()OVER(ORDER BY EmployeeID DESC)Ranks FROM Employees)tbl WHERE Ranks <= 10 ORDER BY EmployeeID ASC 
...
SELECT * FROM (SELECT Salary,DENSE_RANK() OVER(ORDER BY Salary DESC) AS 'Salarys' FROM Employees)tbl WHERE Salarys < 4 
...
SELECT * FROM (SELECT Salary,DENSE_RANK() OVER(ORDER BY Salary DESC) AS 'Salarys' FROM Employees)tbl  
